...
| 原则 | 说明 |
|------|------|
| **设计先行** | 开发新模块或进行重大重构时，**必须**先调用 Superpowers 的 `brainstorming` 技能辅助进行研发设计。以自然语言作为高级编程接口来"编写"系统核心逻辑。<br><br>*流程*：设计 → 审核 → 研发实现 → 自测通过 → 归档。功能研发完成并自测通过后，将 `docs/superpowers/specs/` 或 `docs/superpowers/plans/` 中的设计文档**新增**或**合入** `docs/current` 下的对应模块设计文档（DESIGN_*）。 |
...
| **TDD 实践** | 对新功能、Bug 修复、改变对外行为的重构，默认遵循红—绿—重构。测试类型、目录与工具以 [`SPEC_Test.md`](./docs/current/SPEC_Test.md) 为准。**例外**：一次性原型、生成代码、纯配置等。 |
| **先读后写** | 修改复杂逻辑（认证、升级、启动、构建与打包等）前，**优先读 1～3 篇**最相关文档；不要通读全目录 DESIGN/SPEC。 |
...
| 3 | 验证设计文档 | 确认已有 `docs/current/DESIGN_*.md` 或 `docs/superpowers/specs/` |
| 4 | 遵循实现计划 | 按 `docs/superpowers/plans/` 中的任务清单执行 |
...
| 场景 | 推荐技能 | 说明 |
|------|----------|------|
| 新功能设计 | `brainstorming` | 探索需求、确定方案、输出设计文档 |
| 制定实现计划 | `writing-plans` | 将设计文档拆分为可执行的任务步骤 |
